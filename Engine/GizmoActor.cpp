...
void AGizmoActor::SetSpaceWorldMatrix(EGizmoSpace NewSpace, AActor* PickedActor)
{
    ...
    else if (NewSpace == EGizmoSpace::Local)
    {
        ...
        if (ArrowY)
            ArrowY->
                SetRelativeRotation(TargetRot * FQuat::MakeFromEuler(FVector(0, 0, 90)));
        if (ArrowZ)
            ArrowZ->SetRelativeRotation(
                TargetRot * FQuat::MakeFromEuler(FVector(0, -90, 0)));

        if (ScaleX)
            ScaleX->
                SetRelativeRotation(TargetRot * FQuat::MakeFromEuler(FVector(0, 90, 0)));
        if (ScaleY)
            ScaleY->SetRelativeRotation(
                TargetRot * FQuat::MakeFromEuler(FVector(-90, 0, 0)));
        if (ScaleZ) ScaleZ->SetRelativeRotation(TargetRot * FQuat::MakeFromEuler(FVector(0, 0, 0)));

        if (RotateX)
            RotateX->SetRelativeRotation(
                TargetRot * FQuat::MakeFromEuler(FVector(0, 90, 0)));
        if (RotateY)
            RotateY->SetRelativeRotation(
                TargetRot * FQuat::MakeFromEuler(FVector(90, 0, 0)));
        if (RotateZ)
            RotateZ->SetRelativeRotation(
                TargetRot * FQuat::MakeFromEuler(FVector(0, 0, 0)));
    }
}
...
void AGizmoActor::OnDrag(AActor* Target, uint32 GizmoAxis, float MouseDeltaX, float MouseDeltaY,
                         const ACameraActor* Camera, FViewport* Viewport)
{
    if (!Target || !Camera)
        return;
    ...
    if (CurrentSpace == EGizmoSpace::World)
    {
        switch (GizmoAxis)
        {
        case 1: Axis = FVector(1, 0, 0);
            break;
        case 2: Axis = FVector(0, 1, 0);
            break;
        case 3: Axis = FVector(0, 0, 1);
            break;
        }
    }
    else if (CurrentSpace == EGizmoSpace::Local)
    {
        switch (GizmoAxis)
        {
        case 1: Axis = Target->GetActorRight();
            break; // Local X
        case 2: Axis = Target->GetActorForward();
            break; // Local Y
        case 3: Axis = Target->GetActorUp();
            break; // Local Z
        }
    }
    ...
    case EGizmoMode::Translate:
        {
            ...
            float h = Viewport
                          ...
                          : UInputManager::GetInstance().GetScreenSize().Y;
            if (h <= 0.0f) h = 1.0f;
            float w = Viewport
                          ? static_cast<float>(Viewport->GetSizeX())
                          : UInputManager::GetInstance().GetScreenSize().X;
            float aspect = w / h;
...
void AGizmoActor::ProcessGizmoInteraction(ACameraActor* Camera, FViewport* Viewport,
...
void AGizmoActor::ProcessGizmoHovering(ACameraActor* Camera, FViewport* Viewport,
                                       float MousePositionX, float MousePositionY)
{
    if (!Camera) return;
    ...
    if (!bIsDragging)
        GizmoAxis = CPickingSystem::IsHoveringGizmoForViewport(
            this, Camera, ViewportMousePos, ViewportSize, ViewportOffset, Viewport);
...
void AGizmoActor::ProcessGizmoDragging(ACameraActor* Camera, FViewport* Viewport,
                                       float MousePositionX, float MousePositionY)
{
    if (!TargetActor || !Camera) return;
