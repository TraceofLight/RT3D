	...
	ImGui::BeginChild("ViewportRenderArea", ImVec2(0, ViewportHeight), false, ImGuiWindowFlags_NoScrollbar);
	{
		...
		if (NewWidth > 0 && NewHeight > 0)
		{
			...
			if (PreviewSRV)
			{
				...
				// 뷰포트 영역 저장 (이미지 실제 렌더링 영역)
				PreviewViewportRect.Left = ImagePos.x;
				PreviewViewportRect.Top = ImagePos.y;
				PreviewViewportRect.Right = ImagePos.x + ViewportSize.x;
				PreviewViewportRect.Bottom = ImagePos.y + ViewportSize.y;
		...
		{
			// 유효하지 않은 크기일 경우 PreviewViewportRect 초기화
 ...
 * @brief 전용 렌더 타겟 생성
 */
void SBlendSpace2DEditorWindow::CreateRenderTarget(uint32 Width, uint32 Height)
{
	// 기존 렌더 타겟 해제
	ReleaseRenderTarget();
	...
	if (FAILED(hr))
	{
		UE_LOG("BlendSpace2DEditorWindow: 렌더 타겟 텍스처 생성 실패");
		return;
	}
	...
	if (FAILED(hr))
	{
		UE_LOG("BlendSpace2DEditorWindow: 렌더 타겟 뷰 생성 실패");
		ReleaseRenderTarget();
		return;
	}
	...
	if (FAILED(hr))
	{
		UE_LOG("BlendSpace2DEditorWindow: 셰이더 리소스 뷰 생성 실패");
		ReleaseRenderTarget();
		return;
	}
	...
	if (FAILED(hr))
	{
		UE_LOG("BlendSpace2DEditorWindow: 깊이 스텐실 텍스처 생성 실패");
		ReleaseRenderTarget();
		return;
	}
	...
	if (FAILED(hr))
	{
		UE_LOG("BlendSpace2DEditorWindow: 깊이 스텐실 뷰 생성 실패");
		ReleaseRenderTarget();
		return;
	}
 ...
 * @brief 전용 렌더 타겟 해제
 */
void SBlendSpace2DEditorWindow::ReleaseRenderTarget()
{
 ...
 */
void SBlendSpace2DEditorWindow::UpdateViewportRenderTarget(uint32 NewWidth, uint32 NewHeight)
{
	// 크기가 변경되지 않았으면 스킵
...
/**
 * @brief 블렌드 스페이스 프리뷰를 전용 렌더 타겟에 렌더링
 */
void SBlendSpace2DEditorWindow::RenderToPreviewRenderTarget()
{
	if (!PreviewRenderTargetView || !PreviewDepthStencilView || !PreviewState)
	{
		return;
	}

	if (!PreviewState->Viewport || !PreviewState->Client)
	{
		return;
	}
	...
	// Viewport 크기 업데이트
	PreviewState->Viewport->Resize(0, 0, PreviewRenderTargetWidth, PreviewRenderTargetHeight);

	// 블렌드 스페이스 씬 렌더링
	if (PreviewState->Client)
	{
		PreviewState->Client->Draw(PreviewState->Viewport);
	}
